...
function Update()
{
        ...
        {
                $niceline=str_replace(" => ",";", $line);
                $tmpfacter=explode(";",$niceline);
                $facter[$tmpfacter[0]]=$tmpfacter[1];
        }
        ...
        if($resultarray) {
                $id=$resultarray[0]['id'];
                // Update SW type (OS)
                $osrelease = $facter['operatingsystem'] . $facter['operatingsystemrelease'];
...
function ViewHTML()
{
        ...
        echo "<form method=post enctype=\"multipart/form-data\" action='process.php?page=depot&tab=facter&op=Update'>";
