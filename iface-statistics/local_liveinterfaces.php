<?php
// Render port usage statistics to quickly find not used ports.

$tab['object']['liveinterfaces'] = 'Live interfaces';
$trigger['object']['liveinterfaces'] = 'trigger_liveports';
$tabhandler['object']['liveinterfaces'] = 'renderInterfacesStats';
...
function renderInterfacesStats($object_id)
{
	...
	//get data: show version
        try
        {
                $uptime = gwRetrieveDeviceConfig ($object_id, 'getuptime');
        }
		...
		//get data: show interfaces
                try
                {
                        $interfaces = gwRetrieveDeviceConfig ($object_id, 'getinterfaces');
                }
                catch (RackTablesError $e) {}
                ...
                if (! empty ($interfaces))
                {
                        ...
                        echo "<table width='80%' class='widetable' cellspacing=0 cellpadding='5px' align='center'><tr><th>Port<th>Link status<th>Last input<th>Last clear<th>Input packets<th>Output packets</tr>";
                        $order = 'even';
...
function ios12ReadInterfaces ($input)
{
        ...
        foreach (explode ("\n", $input) as $line)
        {
            ...
            // if text is indented then skip to next interface search
            if ( preg_match ('/^\w.*/', $line)) $state = 'intSearch';
...
$gwrxlator['getuptime'] = array
(
        'ios12' => 'ios12ReadUptime',
);
$gwrxlator['getinterfaces'] = array
(
